...
└─────────────────────────────────────────────────────────────────┘
                              ...
                              ▼ REST API
┌─────────────────────────────────────────────────────────────────┐
│              BACKEND LAYER (FastAPI on Render)                  │
│                                                                 │
│  ┌───────────────────────────────────────────────────────────┐ │
│  │        DOCUMENT INGESTION PIPELINE                        │ │
│  │                                                           │ │
│  │  Upload → Parse → Chunk → Embed → Store                  │ │
│  │  (PDF)   (Text) (1000tk) (768D)  (Qdrant)               │ │
│  └───────────────────────────────────────────────────────────┘ │
│                                                                 │
│  ┌───────────────────────────────────────────────────────────┐ │
...
│                                                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │
│  │   Google     │  │   Qdrant     │  │   Cohere     │         │
│  │   Gemini     │  │   Cloud      │  │  Reranking   │         │
│  │ LLM+Embed    │  │Vector Store  │  │     API      │         │
│  └──────────────┘  └──────────────┘  └──────────────┘         │
└─────────────────────────────────────────────────────────────────┘
```
...
BACKEND
  ...
  │ 4. Chunk into 1000-token segments (120 overlap)
  ▼
GEMINI API
  │
  │ 5. Generate Embeddings (768D vectors)
  ▼
BACKEND
  │
  │ 6. Receive Vectors
...
GEMINI API
  │
  │ 4. Embed Query → 768D Vector
  ▼
QDRANT
  │
  │ 5. Vector Similarity Search → Top 15 Results
  ▼
COHERE API
  │
  │ 6. Rerank Results → Top 5 Most Relevant
  ▼
GEMINI API
  │
...
FRONTEND
  │
  │ 9. Display Answer with Citations
  ▼
USER
```
...
```
            ...
            User Visits Application
              ...
              └───────────────┘
                /           \
               /             \
    Upload Document      Paste Document
         │                     │
         ▼                     ▼
  Upload PDF/TXT/MD    Paste Text Content
         │                     │
         ▼                     ▼
    Process & Embed       Process & Embed
  (Chunk→Embed→Store)  (Chunk→Embed→Store)
         │                     │
         ▼                     ▼
   Document Ready         Content Ready
         │                     │
         └──────────┬──────────┘
                    │
                    ▼
...
**Built for AI/ML Internship Assessment - Track B**
